...
class reg_aladinInputSpec(CommandLineInputSpec):
    ...
    floating_img = File(exists=True, argstr='-flo %s', mandatory=True,
                        position=1, desc='floating image')
    
    #choose one
    option_rigOnly = traits.Bool(argstr = '-rigOnly', 
                               position = 2, desc = 'rigid only flag')
    input_affine_trans = File(exists=True, argstr='-inaff %s',
    ...
    resampled_image = File(exists=True, argstr='-res %s',
